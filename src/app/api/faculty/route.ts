...
export async function POST(req: Request) {
    ...
    try {
        ...
        const { name, fathersName, mothersName, permanentAddress, personalPhoneNumber, fathersPhoneNumber, mothersPhoneNumber, allotedRoomNo, allotedHostel, dateOfJoining, role, email } = body;

        if (!name || !fathersName || !mothersName || !permanentAddress || !personalPhoneNumber || !fathersPhoneNumber || !mothersPhoneNumber || !allotedRoomNo || !allotedHostel || !dateOfJoining || !role || !email) {
...
export async function PUT(req: Request) {
    ...
    try {
        const body = await req.json();
        const { id, name, fathersName, mothersName, permanentAddress, personalPhoneNumber, fathersPhoneNumber, mothersPhoneNumber, allotedRoomNo, allotedHostel, dateOfJoining, role, email } = body;

        if (!id || !name || !fathersName || !mothersName || !permanentAddress || !personalPhoneNumber || !fathersPhoneNumber || !mothersPhoneNumber || !allotedRoomNo || !allotedHostel || !dateOfJoining || !role || !email) {
            return NextResponse.json({ error: "Missing required fields" }, { status: 400 });
        }
